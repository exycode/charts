...
class DataConnector_mysql extends DataConnector
{
    ...
    public function saveToolConsumer($consumer)
    {
        ...
        if (empty($id)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' (consumer_key256, consumer_key, name, ' .
                ...
                DataConnector::quoted($consumer->ltiVersion),
                DataConnector::quoted($consumer->consumerName),
                DataConnector::quoted($consumer->consumerVersion),
                DataConnector::quoted($consumer->consumerGuid),
                DataConnector::quoted($profile),
                DataConnector::quoted($consumer->toolProxy),
    ...
    public function deleteToolConsumer($consumer)
    {
        ...
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . ' WHERE consumer_pk = %d',
            $consumer->getRecordId()
        );
        mysql_query($sql);
        ...
        $sql = sprintf(
            "UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' prl ' .
    ...
    public function saveContext($context)
    {
        ...
        if (empty($id)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' (consumer_pk, lti_context_id, ' .
            ...
            $sql = sprintf(
                "UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' SET ' .
    ...
    public function saveResourceLink($resourceLink)
    {
        ...
        if (empty($id)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' (consumer_pk, context_pk, ' .
    ...
    public function loadConsumerNonce($nonce)
    {
        ...
        // Load the nonce
        $sql = sprintf(
            "SELECT value AS T FROM {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . ' WHERE (consumer_pk = %d) AND (value = %s)',
    ...
    public function saveConsumerNonce($nonce)
    {
        ...
        $sql = sprintf(
            "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . " (consumer_pk, value, expires) VALUES (%d, %s, %s)",
            $nonce->getConsumer()->getRecordId(),
            DataConnector::quoted($nonce->getValue()),
    ...
    public function saveResourceLinkShareKey($shareKey)
    {
        ...
        $sql = sprintf(
            ...
            DataConnector::quoted($shareKey->getId()),
    ...
    public function saveUser($user)
    {
        ...
        if (is_null($user->created)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' (resource_link_pk, ' .
