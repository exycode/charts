...
class DataInput
{
    ...
    {
        $decodeInputData = json_decode($inputData, true);
        ...
        if (!isset($decodeInputData[self::STATUS])) {
            $this->isSafe = false;
            return;
        }

        if ($decodeInputData[self::STATUS] !== self::STATUS_SUCCESS) {
            $this->isSafe = false;
            return;
        }

        $this->isSafe = true;
        $this->inputData = $decodeInputData[self::SAFE_DATA];
    }
